...
import models.mydatasets as mydatasets
...
parser.add_argument('--model_path', help='path of model file (not include the name suffix',
                    type=str, default='snapshots2/')

parser.add_argument('--model_name', help='name of best model',
                    type=str, default='snapshots2/log54_epoch-76_dev-acc-0.937')


parser.add_argument('--cuda', action='store_true', default=True, help='gpu')

parser.add_argument('--train', action='store_true', default=False, help='gpu')

parser.add_argument('--predict', action='store_true', default=True, help='gpu')

args=parser.parse_args()


def make_batch(batch, args):
    src_true, tgt_true = batch.query, batch.doc
    batch_size = src_true.size(0)
    tgt_false = torch.cat((tgt_true[1:], tgt_true[0:1]), dim = 0)

    src_batch = torch.cat((src_true, src_true), dim = 0)
    tgt_batch = torch.cat((tgt_true, tgt_false), dim = 0)
    lbl_batch = torch.cat((torch.ones(batch_size), torch.zeros(batch_size))).type(torch.LongTensor)

    if args.cuda:
        src_batch, tgt_batch, lbl_batch = src_batch.cuda(), tgt_batch.cuda(), lbl_batch.cuda()

    return src_batch, tgt_batch, lbl_batch
...
def train(args):
    ...
    for k in range(args.epoch):
        ...
        for i, batch in enumerate(train_iter):
            train_src_batch, train_tgt_batch, train_lbl_batch = make_batch(batch, args)
        ...
        if (k + 1) % args.dev_interval == 0:
            ...
            for batch in dev_iter:
                dev_src_batch, dev_tgt_batch, dev_lbl_batch = make_batch(batch, args)
...
def predict(args):
    ...
    for i, query in enumerate(query_iter):
        ...
        for j, batch in enumerate(doc_iter):
            ...
            docs_poss = torch.cat((docs_poss, log_prob[:, 1].cpu().data), dim = 0)
...
args.data_path = "data/learn2rank"

tokenize = lambda x: [i for i in x][:args.max_length]
text_field = data.Field(sequential=True, tokenize=tokenize, lower=True, batch_first = True)
train_iter, dev_iter, query_iter, doc_iter = mydatasets.ir_dataset(text_field, args)
